		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						if (leapVictim != null && alignment != leapVictim.alignment){
							if (hit(Ripper.this, leapVictim, Char.INFINITE_ACCURACY, false)) {
						...
						if (endPos != leapPos){
							Actor.add(new Pushing(Ripper.this, leapPos, endPos));
						}

						pos = endPos;
						leapPos = -1;
						sprite.idle();
						Dungeon.level.occupyCell(Ripper.this);
						next();
					}
				});
				return false;
			...
			if (enemyInFOV && !isCharmedBy( enemy ) && canAttack( enemy )) {
				...
				if (enemyInFOV) {
					...
					state = WANDERING;
					target = Dungeon.level.randomDestination( Ripper.this );
					return true;
				}
				...
				if (leapCooldown <= 0 && enemyInFOV && !rooted
					...
					if (b.collisionPos == targetPos){
						...
						if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[leapPos]){
							GLog.w(Messages.get(Ripper.this, "leap"));
