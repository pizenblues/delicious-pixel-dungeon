...
public class RedDragon extends Mob {
	{
	...
	public boolean act() {
		...
		if (Dungeon.level.water[pos] && HP < HT) {
			...
			if (HP*2 > HT) {
				...
				((RedDragonSprite)sprite).spray(false);
				HP = Math.min(HP, HT);
			}
		} else {
			healInc = 1;
	...
	public void updateSpriteState() {
		...
		if (pumpedUp > 0){
			((RedDragonSprite)sprite).pumpUp( pumpedUp );
		}
	}
	...
	protected boolean doAttack( Char enemy ) {
		if (pumpedUp == 1) {
			...
			if (visible) {
				if (pumpedUp >= 2) {
					((RedDragonSprite) sprite).pumpAttack();
				} else {
					sprite.attack(enemy.pos);
				}
			} else {
				if (pumpedUp >= 2){
					((RedDragonSprite)sprite).triggerEmitters();
				}
				attack( enemy );
	...
	public void damage(int dmg, Object src) {
		...
		if ((HP*2 <= HT) && !bleeding){
			...
			((RedDragonSprite)sprite).spray(true);
