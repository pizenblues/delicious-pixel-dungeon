...
public class Griffith extends Mob {

	{
		spriteClass = GriffithSprite.class;
				...
				if (enemy != null && enemy.isAlive() && !canReach){
					...
					if (turnsSinceLastAbility >= MIN_COOLDOWN){
						...
						//unless enemy is already stunned, we don't want to stunlock them
							...
							turnsSinceLastAbility = 0;
							if (sprite != null && (sprite.visible || enemy.sprite.visible)) {
								((GriffithSprite)sprite).slam(enemy.pos);
								return false;
							} else {
								dropRocks(enemy);
	...
	public void supercharge(){
		...
		((GriffithSprite)sprite).updateChargeState(true);
		((GriffithSprite)sprite).charge();
	...
	public void loseSupercharge(){
		supercharged = false;
		((GriffithSprite)sprite).updateChargeState(false);
