...
public class Monstrosity extends Mob {

	{
		spriteClass = MonstrositySprite.class;
	...
	private static Class getPairedFist(Class fist){
		if (fist == MonstrosityMinion.BurningFist.class) return MonstrosityMinion.SoiledFist.class;
		if (fist == MonstrosityMinion.SoiledFist.class) return MonstrosityMinion.BurningFist.class;
		if (fist == MonstrosityMinion.RottingFist.class) return MonstrosityMinion.RustedFist.class;
		if (fist == MonstrosityMinion.RustedFist.class) return MonstrosityMinion.RottingFist.class;
		if (fist == MonstrosityMinion.BrightFist.class) return MonstrosityMinion.DarkFist.class;
		if (fist == MonstrosityMinion.DarkFist.class) return MonstrosityMinion.BrightFist.class;
		return null;
	}
	...
	private ArrayList<Class> challengeSummons = new ArrayList<>();
	{
		...
		Random.pushGenerator(Dungeon.seedCurDepth()+1);
			fistSummons.add(Random.Int(2) == 0 ? MonstrosityMinion.BurningFist.class : MonstrosityMinion.SoiledFist.class);
			fistSummons.add(Random.Int(2) == 0 ? MonstrosityMinion.RottingFist.class : MonstrosityMinion.RustedFist.class);
			fistSummons.add(Random.Int(2) == 0 ? MonstrosityMinion.BrightFist.class : MonstrosityMinion.DarkFist.class);
		...
		if (Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
			for (int i = 0; i < 6; i++){
				if (i >= 4){
					...
					regularSummons.add( i % 2 == 0 ? YogEye.class : BPMinion.class);
				}
			}
		} else {
			for (int i = 0; i < 4; i++){
	...
	public void damage( int dmg, Object src ) {
		...
		if (phase < 4 && HP <= HT - 300*phase){
			...
			addFist((MonstrosityMinion)Reflection.newInstance(fistSummons.remove(0)));

			if (Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
				addFist((MonstrosityMinion)Reflection.newInstance(challengeSummons.remove(0)));
			}
	...
	public void addFist(MonstrosityMinion fist){
	...
	private MonstrosityMinion findFist(){
		for ( Char c : Actor.chars() ){
			if (c instanceof MonstrosityMinion){
				return (MonstrosityMinion) c;
			}
		}
		return null;
	}
	...
	public void aggro(Char ch) {
		for (Mob mob : (Iterable<Mob>)Dungeon.level.mobs.clone()) {
			if (mob != ch && Dungeon.level.distance(pos, mob.pos) <= 4 &&
					(mob instanceof Larva || mob instanceof YogRipper || mob instanceof YogEye || mob instanceof BPMinion)) {
				mob.aggro(ch);
			}
		}
	}
	...
	public void die( Object cause ) {
		...
		for (Mob mob : (Iterable<Mob>)Dungeon.level.mobs.clone()) {
			if (mob instanceof Larva || mob instanceof YogRipper || mob instanceof YogEye || mob instanceof BPMinion) {
				mob.die( cause );
			}
		}
	...
	public static class YogRipper extends Ripper {
		{
			maxLvl = -2;
			properties.add(Property.BOSS_MINION);
		}
	...
	public static class BPMinion extends BlackPhillip {
		{
			maxLvl = -2;
			properties.add(Property.BOSS_MINION);
		}
