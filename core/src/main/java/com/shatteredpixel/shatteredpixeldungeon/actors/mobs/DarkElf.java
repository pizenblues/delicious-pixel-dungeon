...
public class DarkElf extends Mob {
	...
	{
		spriteClass = DarkElfSprite.class;
			...
			if (delay <= 0){
				...
				if (Actor.findChar(pos) == null) {
					...
					if (!ch.isAlive() && ch == Dungeon.hero) {
						Dungeon.fail(DarkElf.class);
						GLog.n( Messages.capitalize(Messages.get(Char.class, "kill", Messages.get(DarkElf.class, "name"))));
					}
				}
